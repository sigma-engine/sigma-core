...
namespace opengl {
    ...
    g_buffer::g_buffer(glm::ivec2 size)
		...
		, depth_texture_(0)
		//, depth_buffer_(0)
		//, default_render_buffer_(0)
    {
		// TODO look into render buffers

        // Get the default draw frame buffer
		//GL_CHECK(glGetIntegerv(GL_RENDERBUFFER_BINDING, &default_render_buffer_));
        GL_CHECK(glGetIntegerv(GL_FRAMEBUFFER_BINDING, &default_fbo_));

		//GL_CHECK(glGenRenderbuffers(1,&depth_buffer_));
		//GL_CHECK(glBindRenderbuffer(GL_RENDERBUFFER, depth_buffer_));
		//GL_CHECK(glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, size.x, size.y));
		//GL_CHECK(glBindRenderbuffer(GL_RENDERBUFFER, default_render_buffer_));
		...
		//GL_CHECK(glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depth_buffer_));
