...
function(add_resources RESOURCE_TARGET RESOURCE_PACKAGE_DIRECTORY)
    ...
    foreach(SOURCE_FILE ${SOURCE_FILES})
        ...
        elseif("${SOURCE_EXTENTION}" STREQUAL ".blend")
            ...
            set(RESOURCE_COMMAND_DEPENDS "${CMAKE_SOURCE_DIR}/tools/mdlcc/blendcc.py")
        ...
        elseif("${SOURCE_EXTENTION}" STREQUAL ".tesc")
            set(RESOURCE_TYPE "tessellation_control")
            set(RESOURCE_COMMAND shdcc ${RESOURCE_INCLUDE_ARGUMENTS})
            set(RESOURCE_COMMAND_DEPENDS)
        elseif("${SOURCE_EXTENTION}" STREQUAL ".tese")
            set(RESOURCE_TYPE "tessellation_evaluation")
            set(RESOURCE_COMMAND shdcc ${RESOURCE_INCLUDE_ARGUMENTS})
            set(RESOURCE_COMMAND_DEPENDS)
        ...
        list(APPEND OUTPUT_${RESOURCE_TYPE} "${RESOURCE_OUTPUT_DIRECTORY}/${RESOURCE_IDENTIFIER}")
        add_custom_command(
    ...
    add_custom_target(${RESOURCE_TARGET}-shaders ALL DEPENDS shdcc ${OUTPUT_vertex} ${OUTPUT_tessellation_control} ${OUTPUT_tessellation_evaluation} ${OUTPUT_geometry} ${OUTPUT_fragment})
    add_custom_target(${RESOURCE_TARGET}-materials ALL DEPENDS mtlcc ${RESOURCE_TARGET}-textures ${RESOURCE_TARGET}-cubemaps ${RESOURCE_TARGET}-shaders ${OUTPUT_material})
    add_custom_target(${RESOURCE_TARGET}-static-meshes ALL DEPENDS mdlcc ${RESOURCE_TARGET}-materials ${OUTPUT_static_mesh})
    add_custom_target(${RESOURCE_TARGET}-post-process-effects ALL DEPENDS effcc ${RESOURCE_TARGET}-textures ${RESOURCE_TARGET}-cubemaps ${RESOURCE_TARGET}-shaders ${OUTPUT_post_process_effect})
    add_custom_target(${RESOURCE_TARGET}-blueprints ALL DEPENDS ${RESOURCE_TARGET}-static-meshes ${OUTPUT_blueprint})
endfunction()
