...
int main(int argc, char const* argv[])
{
    ...
    if (vm.count("input-files") <= 0) {
        std::cerr << "model-compiler: fatal error: no input files." << std::endl;
        return -1;
    }
    ...
    for (auto file_path :
        ...
        if (sigmafive::util::directory_contains_file(boost::filesystem::current_path(), file_path)) {
            if (boost::filesystem::exists(file_path)) {
                sigmafive::convert::assimp_converter imported{ file_path };
                for (auto mesh_name : imported.static_mesh_names()) {
                    sigmafive::resource::development_identifier rid("static_mesh", file_path, mesh_name);
                    auto final_path = outputdir / std::to_string(rid.value());

                    sigmafive::graphics::static_mesh mesh;
