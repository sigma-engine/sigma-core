...
int main(int argc, char const* argv[])
{
    ...
    ("output,l", "List outputs of the post process effect conversion.")
    ("dependency,M", "List dependencies of the post process effect conversion.")
    ("source-file,c", boost::program_options::value<std::string>()->required(), "The post process effect file to convert.");
    // clang-format on
    ...
    try {
        ...
        sigma::compile_shader_technique(effect, settings);

        if (settings.isMember("static_mesh"))
            effect.mesh = boost::filesystem::path{ "static_mesh" } / settings["static_mesh"].asString();
