    ...
    for (auto file_path : vm["input-files"].as<std::vector<boost::filesystem::path> >()) {
        ...
        if (sigmafive::util::directory_contains_file(boost::filesystem::current_path(), file_path)) {
            if (boost::filesystem::exists(file_path)) {
                sigmafive::resource::development_identifier rid("texture", file_path);
                ...
                sigmafive::graphics::texture texture;
                ...
                std::cerr << "texture-compiler: error: file '" << file_path << "' does not exist!" << std::endl;
            }
        } else {
            std::cerr << "texture-compiler: error: file '" << file_path << "' is not contained in '" << boost::filesystem::current_path() << "'!" << std::endl;
        }
    }
